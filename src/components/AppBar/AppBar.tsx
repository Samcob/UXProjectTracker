...
const AppBar = ({ isOpen, setIsOpen }: IAppbarProps) => {
    return(
        <>
            <div className="flex justify-between items-center mb-4 pl-4 pt-4 pb-4 bg-primary-color">
                ...
                <div className="pr-4 md:flex md:items-center">
                        ...
                        <nav className="pt-2" aria-label="Appbar Navigation">
                            {primaryNavLinks.map((link, index) => (
                                <NavLink
                                    ...
                                    to={link.href}
                                    className={({ isActive }) =>
                                        `mx-2 w-fit border-b-2 border-primary-color p-2 text-white font-bold hover:border-b-secondary-font-color hover:font-bold  ${isActive ? 'border-secondary-font-color' : ''
                                        }`
                                    }
                                >
                                    {link.name}
                    ...
                    <div>
                        <button className-="flex-none " onClick={() => setIsOpen( !isOpen )}>
                            ...
                            <div className=" w-full text-center text-s font-medium capitalize text-secondary-font-color">
                                Menu
                            </div>
                        </button>
                    </div>
